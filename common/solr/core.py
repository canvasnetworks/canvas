...
class SearchHandler(object):
    ...
    def __call__(self, q=None, fields=None, highlight=None,
                 ...
                 defType=None, debug=False, **params):
        """
        ...
        xml = self.raw(**params)
        return parse_query_response(StringIO(xml),  params, self, debug)
...
class Response(object):
    ...
    def __init__(self, query):
        ...
        # Overriden by parser if debug flag is True
        self.debug = None
...
# XML Parsing support
# ===================================================================
def parse_query_response(data, params, query, debug=False):
    """
    ...
    parser.parse(data)
    root = handler.stack[0]
    if root.children:
        response = root.children[0].final
        response._params = params
        response._query = query
        
        if debug and len(root.children) > 1:
            response.debug = root.children[1].final
