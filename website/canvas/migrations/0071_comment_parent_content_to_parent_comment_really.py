...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for comment in orm.Comment.objects.all():
            ...
            # We are migrating parents from Content to Comment, so if they don't have a parent, nothing to do.
            if parent_content is None:
                continue
                
            try:
                [parent_comment] = parent_content.used_in_comments.order_by('timestamp')[:1]
            except ValueError:
                print "Creating new comment for ", parent_content.id
                parent_comment = orm.Comment(
                    anonymous=True,
                    reply_content=parent_content,
