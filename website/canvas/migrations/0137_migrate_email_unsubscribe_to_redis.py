...
class Migration(DataMigration):
    """ We used to store unsubscribe data by email in EmailUnsubscribe. We've now
    moved to per user email unsubscriptions. This stipulates that if email is not none,
    we should have only one user corresponding to it.
    """
    def forwards(self, orm):
        # @note: This a one-time migration script. Hence, I've commented the actual migration out as to not break the build if we change the underlying 
        # models.
        """
        for row in orm.EmailUnsubscribe.objects.all():
            email = row.email
            try:
                user = CanvasUser.get_by_email(email)
                assert user
            except Exception, e:
                print e
                print "Did not find a user corresponding to %s" % email
                continue
            user.kv.subscriptions.unsubscribe_from_all(EmailChannel)
            print "Unsubscribed %s" % email
        """
