...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        User = orm['auth.User']
        try:
            canvas = User.objects.get(username='canvas')
        except User.DoesNotExist:
            canvas = User(username='canvas')
            canvas.save()
            
            orm.UserInfo(user=canvas).save()
            
        orm.Category(name="bookmarklet", description="Images stickered via the bookmarklet automatically appear here.", founder=canvas).save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.Category.objects.get(name="bookmarklet").delete()
