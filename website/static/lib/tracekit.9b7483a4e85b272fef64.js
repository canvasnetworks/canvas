...
TraceKit.report = (function () {
    ...
    window.onerror = function (message, url, lineNo) {
        if (lastHandledException) {
            lastHandledException = null;
            return false; // don't let it bubble up and actually cause an error in the browser after the first time.
        }
        var stack = null;
        ...
        // let it bubble up and actually cause an error in the browser
        return;
    };
...
TraceKit.computeStackTrace = (function () {
    ...
    function getSource(url) {
        // Don't bother retrieving the source anymore. Useless since we minify JS.
        //return '';
        if (!sourceCache.hasOwnProperty(url)) {
            var source;
            // Check same origin.
            if (document.location.host != parseUrl(url).host) {
                source = '';
            } else {
...
(function (w) {
	var _helper = function(fnName) {
		...
		w[fnName] = function() {
            ...
            // Don't try to process string "callbacks" so that we don't have to eval stuff ourselves.
            if (originalCallback && originalCallback.apply) {
